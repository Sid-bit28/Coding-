...
lli rec(int pos,int times,int last){
    if(pos==n){
        if(times<0)return -INF;
        return 0;
    }
    if(times<0){
        return -INF;
    ...
    if(dp[pos][times][last]!=-1)return dp[pos][times][last];
    
    lli ans=-INF;
    fr(i,3){
        if(arr[pos]=='H' && i==1){
            ans=max(ans,1+rec(pos+1,times-(i==last?0:1),i));
        }else if(arr[pos]=='P' && i==2){
            ans=max(ans,1+rec(pos+1,times-(i==last?0:1),i));
        }else if(arr[pos]=='S' && i==0){
            ans=max(ans,1+rec(pos+1,times-(i==last?0:1),i));
        }else {
            ans=max(ans,rec(pos+1,times-(i==last?0:1),i));
        }
    }
    return dp[pos][times][last]=ans;
}
void solve(){
