...
using vii = vector<pii>;  using graph = vector<vi>;
const int MOD = 1e9+7;    const int INF = 1e18;
using ld = long double;   const ld EPS = 1e-9;
#ifndef SIDDARTH
...
void solve(){
	...
	brr[0]=arr[0];brr.back()=arr.back();
	rep(i,1,n-1){
		brr[i]=(arr[i]*arr[i-1])/__gcd(arr[i],arr[i-1]);
	}
	// pr(brr);
	bool ok=true;
	fr(i,n){
		if(__gcd(brr[i],brr[i+1])!=arr[i])ok=false;
	}
	cout<<(ok?"YES":"NO")<<endl;
}
