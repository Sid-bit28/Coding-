...
void solve(){
	...
	fr(i,n){
		int x;cin>>x;
		mp[arr[i]].push_back(x);
	}
	sort(all(mp[0]),greater<int>());sort(all(mp[1]),greater<int>());
	int ans=0;
	bool ok=false;
	while(sz(mp[0]) != sz(mp[1])){
		ok=true;
		if(sz(mp[0]) > sz(mp[1])){
			ans+=mp[0].back();mp[0].pop_back();
		}else {
			ans+=mp[1].back();mp[1].pop_back();
		}
	}
	for(auto x:mp[0]){
		ans+=2*x;
	...
	if(!ok){
		ans-=(*mp[0].rbegin()<=*mp[1].rbegin()?*mp[0].rbegin():*mp[1].rbegin());
	}
	cout<<ans<<endl;
}
