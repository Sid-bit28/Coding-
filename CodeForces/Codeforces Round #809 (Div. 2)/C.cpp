...
int rec(int pos,int chor){
	if(pos>=n-1){
		return 0;
	}
	
	if(dp[pos][chor]!=-1)return dp[pos][chor];
	
	int ans=INF;
	if(chor){
		ans=min(ans,max(0LL,max(arr[pos+1],arr[pos-1])-arr[pos]+1)+rec(pos+2,1));
	}else {
		ans=min({ans,rec(pos+1,1),max(0LL,max(arr[pos+1],arr[pos-1])-arr[pos]+1)+rec(pos+2,0)});
	}
	return dp[pos][chor]=ans;
}
void solve(){
	cin>>n;
	fr(i,n){
